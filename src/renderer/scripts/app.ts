...
class MagicAPIApp {
    ...
    private setupUI(): void {
        ...
        // Create a new workflow by default
        this.newWorkflow();
    }
    ...
    private setupIPCListeners(): void {
        // Menu commands
        ipcRenderer.on('menu-new-workflow', () => this.newWorkflow());
        ipcRenderer.on('menu-open-workflow', () => this.openWorkflow());
        ipcRenderer.on('menu-save-workflow', () => this.saveWorkflow());
        ipcRenderer.on('menu-save-as-workflow', () => this.saveAsWorkflow());
        ipcRenderer.on('menu-run-workflow', () => this.runWorkflow());
        ipcRenderer.on('menu-stop-workflow', () => this.stopWorkflow());
        ipcRenderer.on('menu-validate-workflow', () => this.validateWorkflow());
    }
    ...
    private async openWorkflow(): Promise<void> {
        try {
            ...
            if (!result.canceled && result.filePaths.length > 0) {
                ...
                this.log('info', `Opened workflow: ${result.filePaths[0]}`);
            }
        } catch (error) {
    ...
    private async saveWorkflow(): Promise<void> {
        ...
        try {
            ...
            if (!result.canceled && result.filePath) {
                ...
                // Implementation will be added later
                this.log('info', `Saved workflow: ${result.filePath}`);
            }
        } catch (error) {
    ...
    private runWorkflow(): void {
        ...
        // Execution implementation will be added later
    }
    ...
    private validateWorkflow(): void {
        ...
        this.currentWorkflow.nodes.forEach(node => {
            const validation = NodeFactory.validateNode(node);
            node.validation = validation;
            if (!validation.isValid) {
                hasErrors = true;
                this.log('error', `Validation error in ${node.metadata.title}: ${validation.errors[0].message}`);
            }
        });
